...
export default function ContactClient() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await fetch('/api/inquiries', { method: 'POST', body: JSON.stringify(payload), headers: { 'Content-Type': 'application/json' } })
    setSubmitting(false)
    if (res.ok) {
      setDone(true)
    } else {
  ...
  if (done) {
    return (
      <main className="mx-auto max-w-2xl px-6 py-16">
        <h1 className="text-3xl font-semibold">{t(lang, 'form_success_title')}</h1>
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16">
      <div className="rounded-2xl overflow-hidden border border-white/10">
        <img src="https://images.unsplash.com/photo-1516321497487-e288fb19713f?q=80&w=1600&auto=format&fit=crop" alt="Contact" className="w-full h-[240px] object-cover" />
      </div>
      <h1 className="text-4xl font-semibold mt-10">{t(lang, 'nav_contact')}</h1>
      <p className="mt-2 text-white/70">{t(lang, 'meta_contact_desc')}</p>
      ...
      <form onSubmit={onSubmit} className="mt-8 grid gap-4 rounded-2xl border border-white/10 bg-white/5 p-6">
        ...
        <button disabled={submitting} className="mt-2 rounded-md bg-cyan-500 px-6 py-3 text-black hover:bg-cyan-400 disabled:opacity-60">
